...
class TestingController extends Controller
{
    ...
    public function checkLogicGeofence(Request $request){		
        try{
            ...
            //  self::checkVehicleActivity($request->all());
             ...
             // check checkGeofence
            self::checkGeofence($vehicle, $request->all());
    ...
    private function checkGeofence($vehicle, $param){
        ...
        $geofence   = MasterGeofence::join('MsGeofenceVehicle', 'MsGeofenceVehicle.geofence_id', '=', 'MsGeofence.id')
                                     ->where('MsGeofenceVehicle.vehicle_id', $vehicle['id'])
                                     ->select('MsGeofence.*','MsGeofenceVehicle.alert_in_zone','MsGeofenceVehicle.alert_out_zone')
                                     ->get();

        if(!empty($geofence)) foreach($geofence as $gf){
            $geoDetail = MasterGeofenceDetail::where('geofence_id', $gf->id)->get();
            if(!empty($geoDetail)) foreach($geoDetail as $detail)
                $polygon[] = [$detail->latitude, $detail->longitude];
            
            if(!empty($polygon)){
                self::$temp['geofence_name'] = $gf->geofence_name;
                self::$temp['geofence_type'] = $gf->type;
                if($this->checkPolygon($point, $polygon)){
                    self::$temp['out_of_geofence'] = FALSE;
                }else{
                    self::$temp['out_of_geofence'] = TRUE;
                }
                MwMappingGeofence::create(self::$temp);
            }
        }
    ...
    private function checkVehicleActivity($param){
        ...
        // Stop
        if($param['ignition'] == 0  && $param['speesd'] <= 0)
